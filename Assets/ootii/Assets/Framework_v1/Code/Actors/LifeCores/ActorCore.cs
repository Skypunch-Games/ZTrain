...
namespace com.ootii.Actors.LifeCores
{
    ...
    {
        /// <summary>
        /// Default string values to use when defining actor core states
        /// </summary>
        public static class DefaultStates
        {
            public static readonly string DefaultForm = "Default Form";
            public static readonly string CurrentForm = "Current Form";
            public static readonly string Stance = "Stance";
        }
        ...
        public void InstantiateStates()
        {
            ...
            // Ensure our list is distinct
            for (int i = _States.Count - 1; i >= 1; i--)
            {
                for (int j = i - 1; j >= 0; j--)
                {
                    if (_States[i]._Name == _States[j]._Name)
                    {
                        _States.RemoveAt(i);
                        break;
                    }
            ...
            {
                if (!mStateHash.ContainsKey(_States[i]._Name))
                {
        ...
        /// <summary>
        /// Sets the initial value of the specified state variable; if the state already exists, then the value won't
        /// be set (as we are assuming we don't want to overwrite a value that has already been set)
        /// </summary>        
        /// <param name="rName"></param>
        /// <param name="rValue"></param>
        public void SetInitialStateValue(string rName, int rValue)
        {
            if (!StateExists(rName)) { SetStateValue(rName, rValue); }
        }
        ...
        public void InstantiateEffects()
        {
            ...
            for (int i = 0; i < lDefCount; i++)
            {
                ...
                bool lUpdateType;
                Type lType = JSONSerializer.GetType(lDefinition, "__Type", out lUpdateType);
                ...
                if (lEffect != null)
                {
                    ...
                    // CDL - 07/27/2018 - update AssemblyQualifiedName if required
                    if (lUpdateType)
                    {
                        _EffectDefinitions[i] = lEffect.Serialize();
                    }
                }
            }
