...
namespace AwesomeTechnologies
{
    ...
    public class MeshUtils
    {
        ...
        public static GameObject SelectMeshObject(GameObject go, MeshType meshType, int lodIndex)
        {
            switch (meshType)
            {
                ...
                case MeshType.Billboard:
                    return SelectBillboardMesh(go);
            }

            return null;
        }

        public static Quaternion GetMeshRotation(GameObject go, int lodIndex)
        {
            GameObject meshGameObject = SelectMeshObject(go, MeshType.Normal, lodIndex);
            if (meshGameObject)
            {
        ...
        static GameObject SelectNormalMesh(GameObject go, int lodIndex)
        {
            ...
            //if (lodGroup)
            //{
            //    if (lodIndex == 0 && lodGroup.lodCount > 2)
            //    {
            //        LOD[] lods = lodGroup.GetLODs();                   
            //        if (lods[2].renderers[0] && lods[2].renderers[0] is MeshRenderer)
            //        {
            //            Debug.Log("found " + lods[2].renderers[0].gameObject.name);
            //            return lods[2].renderers[0].gameObject;
            //        }
            //    }

            //    if (lodIndex == 1 && lodGroup.lodCount > 2)
            //    {
            //        LOD[] lods = lodGroup.GetLODs();
            //        if (lods[2].renderers[0] && lods[2].renderers[0] is MeshRenderer)
            //        {
            ...
            //    if (lodIndex == 2 && lodGroup.lodCount > 1)
            //    {
            //        LOD[] lods = lodGroup.GetLODs();
            //        if (lods[1].renderers[0] && lods[1].renderers[0] is MeshRenderer)
            //        {
            //            Debug.Log("found " + lods[1].renderers[0].gameObject.name);
            //            return lods[2].renderers[0].gameObject;
            //        }
            //    }
            ...
            //    {
            //        LOD[] lods = lodGroup.GetLODs();
            //        if (lods[0].renderers[0] && lods[0].renderers[0] is MeshRenderer)
            //        {
            //            Debug.Log("found " + lods[0].renderers[0].gameObject.name);
            //            return lods[2].renderers[0].gameObject;
            //        }
            //    }
            //}
            ...
            if (lodIndex == 0)
            {            
            selectedObject = FindMeshWithNamePart("LOD2", go);
            if (selectedObject) return selectedObject;
            }

            if (lodIndex <=1)
            {
                selectedObject = FindMeshWithNamePart("LOD1", go);
                if (selectedObject) return selectedObject;
            }

            selectedObject = FindMeshWithNamePart("LOD0", go);
            if (selectedObject) return selectedObject;
        ...
        static GameObject FindMeshWithNamePart(string namePart, GameObject go)
        {                      
            for (var i = 0; i <= go.transform.childCount -1; i++)
            {               
                if (go.transform.GetChild(i).gameObject.name.ToUpper().Contains(namePart.ToUpper()))
                {
        ...
        static GameObject SelectBillboardMesh(GameObject go)
        {
            var selectedObject = FindMeshWithNamePart("Billboard", go);
            if (selectedObject) return selectedObject;
            return null;
        }
