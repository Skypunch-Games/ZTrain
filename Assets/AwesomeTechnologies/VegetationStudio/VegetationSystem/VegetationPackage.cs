...
namespace AwesomeTechnologies
{
    ...
    public class MaskTextureInfo
    {
        ...
        public Vector2 Uv = new Vector2(15, 15);
    ...
    public class VegetationPackage : ScriptableObject
    {
        ...
        public void LoadTexturesFromTerrain(TerrainData sourceTerrainData)
        {
                    ...
                    TerrainLayer[] terrainLayers = sourceTerrainData.terrainLayers;

                    for (int j = 0; j <= TerrainTextureList.Count - 1; j++)
                    {
                        if (j < terrainLayers.Length)
                        {
                            TerrainTextureList[j].Texture = terrainLayers[j].diffuseTexture;
                            TerrainTextureList[j].TextureNormals = terrainLayers[j].normalMapTexture;
                            TerrainTextureList[j].offset = terrainLayers[j].tileOffset;
                            TerrainTextureList[j].TileSize = terrainLayers[j].tileSize;
                        }
                    }
#else
